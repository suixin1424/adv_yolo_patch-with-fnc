...
im = PIL.Image.open('/home/zhuyiming/ssd2/zhuyiming/yolov2-patch/adversarial-yolo/data/horse.jpg').convert('RGB')
print('img read!')
...
class InriaDataset(Dataset):
    ...
    def pad_and_scale(self, img, lab):
        ...
        if w==h:
            ...
            if dim_to_pad == 1:
                padding = (h - w) // 2
                padded_img = PIL.Image.new('RGB', (h,h), color=(127,127,127))
                padded_img.paste(img, (int(padding), 0))
                lab[:, [1]] = (lab[:, [1]] * w + padding) / h
                lab[:, [3]] = (lab[:, [3]] * w / h)
            else:
                padding = (w - h) // 2
                padded_img = PIL.Image.new('RGB', (w, w), color=(127,127,127))
